defmodule SymphonyElixir.Codex.AppServer do
  ...
  def start_session(workspace, opts \\ []) do
    ...
    profile = Keyword.get(opts, :profile) || Config.agent_profile!(:executor)

    with {:ok, expanded_workspace} <- WorkspaceGuard.validate(workspace, worker_host),
         {:ok, port} <- start_port(expanded_workspace, worker_host, profile) do
      metadata = port_metadata(port, worker_host)

      with {:ok, session_policies} <- session_policies(profile, expanded_workspace, worker_host),
  ...
  defp start_port(workspace, nil, profile) do
      ...
      port =
        Port.open(
          ...
          [
            ...
            :exit_status,
            :stderr_to_stdout,
            args: [~c"-lc", String.to_charlist(profile.command)],
  ...
  defp start_port(workspace, worker_host, profile) when is_binary(worker_host) do
    remote_command = remote_launch_command(workspace, profile)
    SSH.start_port(worker_host, remote_command, line: @port_line_bytes)
  end

  defp remote_launch_command(workspace, profile) when is_binary(workspace) do
    [
  ...
  defp session_policies(profile, workspace, nil) do
    Config.agent_runtime_settings(profile, workspace)
  end

  defp session_policies(profile, workspace, worker_host) when is_binary(worker_host) do
    Config.agent_runtime_settings(profile, workspace, remote: true)
  end
